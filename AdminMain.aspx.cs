    ...
    public partial class AdminMain : BasePage
    {
        ...
        private void HandlePost()
        {
            ...
            else if (action == "photo_add")
            {
                ...
                // 检查该用户是否已经是摄影师
                object exists = DBHelper.ExecuteScalar("select count(*) from photographer where user_id=@uid", new MySqlParameter("@uid", uid));
                if (Convert.ToInt32(exists) > 0)
                {
                    Alert("该用户已经是摄影师，不能重复添加。");
                    return;
                }
                DBHelper.ExecuteNonQuery("insert into photographer(user_id,nick_name,avatar,specialty,price,intro,free_time,status,create_time) values(@uid,@nick,'Images/work-portrait.png',@sp,@price,@intro,@free,1,now())", new MySqlParameter("@uid", uid), new MySqlParameter("@nick", FormText("nick_name")), new MySqlParameter("@sp", FormText("specialty")), new MySqlParameter("@price", ToDecimal(FormText("price"), 0)), new MySqlParameter("@intro", FormText("intro")), new MySqlParameter("@free", FormText("free_time")));
                // 同时更新用户角色为摄影师
        ...
        private string Sel(object current, string value)
        {
            return current.ToString() == value ? " selected" : "";
        }
        ...
        private void LoadWorks()
        {
            ...
            for (int i = 0; i < table.Rows.Count; i++)
            {
                ...
                html.Append("<tr><td>" + HtmlEncode(r["title"]) + "</td><td>" + HtmlEncode(r["nick_name"]) + "</td><td>" + HtmlEncode(r["work_type"]) + "</td><td>" + HtmlEncode(r["audit_status"]) + "</td><td>" + HtmlEncode(r["is_recommend"]) + "</td><td><form method=\"post\" action=\"AdminMain.aspx\" class=\"inline-form\"><input type=\"hidden\" name=\"action\" value=\"work_audit\" /><input type=\"hidden\" name=\"work_id\" value=\"" + r["work_id"] + "\" /><select name=\"audit_status\"><option value=\"1\"" + Sel(r["audit_status"], "1") + ">通过</option><option value=\"0\"" + Sel(r["audit_status"], "0") + ">待审</option></select><select name=\"is_recommend\"><option value=\"0\"" + Sel(r["is_recommend"], "0") + ">不推荐</option><option value=\"1\"" + Sel(r["is_recommend"], "1") + ">推荐</option></select><button class=\"btn-light btn\" type=\"submit\">保存</button></form><form method=\"post\" action=\"AdminMain.aspx\" class=\"inline-form\"><input type=\"hidden\" name=\"action\" value=\"work_delete\" /><input type=\"hidden\" name=\"work_id\" value=\"" + r["work_id"] + "\" /><button class=\"btn-light btn\" type=\"submit\">删除</button></form></td></tr>");
            }
            html.Append("</table>");
        ...
        private void LoadPhotographers()
        {
            ...
            {
                DataRow r = table.Rows[i];
                html.Append("<tr><form method=\"post\" action=\"AdminMain.aspx\"><td><input class=\"input\" name=\"nick_name\" value=\"" + HtmlEncode(r["nick_name"]) + "\" /></td><td><input class=\"input\" name=\"specialty\" value=\"" + HtmlEncode(r["specialty"]) + "\" /></td><td><input class=\"input\" name=\"price\" value=\"" + HtmlEncode(r["price"]) + "\" /></td><td><select name=\"status\"><option value=\"1\"" + Sel(r["status"], "1") + ">启用</option><option value=\"0\"" + Sel(r["status"], "0") + ">禁用</option></select></td><td><input type=\"hidden\" name=\"action\" value=\"photo_save\" /><input type=\"hidden\" name=\"photographer_id\" value=\"" + r["photographer_id"] + "\" /><input type=\"hidden\" name=\"intro\" value=\"" + HtmlEncode(r["intro"]) + "\" /><input type=\"hidden\" name=\"free_time\" value=\"" + HtmlEncode(r["free_time"]) + "\" /><button class=\"btn-light btn\" type=\"submit\">保存</button></form><form method=\"post\" action=\"AdminMain.aspx\" class=\"inline-form\"><input type=\"hidden\" name=\"action\" value=\"photo_delete\" /><input type=\"hidden\" name=\"photographer_id\" value=\"" + r["photographer_id"] + "\" /><button class=\"btn-light btn\" type=\"submit\">删除</button></form></td></tr>");
            }
            html.Append("</table>");
        ...
        private void LoadOrders()
        {
            ...
            {
                DataRow r = table.Rows[i];
                html.Append("<tr><form method=\"post\" action=\"AdminMain.aspx\"><td>" + HtmlEncode(r["user_name"]) + "</td><td>" + HtmlEncode(r["nick_name"]) + "</td><td><input class=\"input\" name=\"shoot_date\" value=\"" + Convert.ToDateTime(r["shoot_date"]).ToString("yyyy-MM-dd") + "\" /></td><td><input class=\"input\" name=\"shoot_place\" value=\"" + HtmlEncode(r["shoot_place"]) + "\" /></td><td><select name=\"order_status\"><option" + Sel(r["order_status"], "待确认") + ">待确认</option><option" + Sel(r["order_status"], "已确认") + ">已确认</option><option" + Sel(r["order_status"], "已完成") + ">已完成</option><option" + Sel(r["order_status"], "已取消") + ">已取消</option></select></td><td><input type=\"hidden\" name=\"action\" value=\"order_save\" /><input type=\"hidden\" name=\"order_id\" value=\"" + r["order_id"] + "\" /><input type=\"hidden\" name=\"requirement\" value=\"" + HtmlEncode(r["requirement"]) + "\" /><button class=\"btn-light btn\" type=\"submit\">保存</button></form><form method=\"post\" action=\"AdminMain.aspx\" class=\"inline-form\"><input type=\"hidden\" name=\"action\" value=\"order_delete\" /><input type=\"hidden\" name=\"order_id\" value=\"" + r["order_id"] + "\" /><button class=\"btn-light btn\" type=\"submit\">删除</button></form></td></tr>");
            }
            html.Append("</table>");
